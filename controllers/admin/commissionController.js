...
const editUserQuantityBasedCommissionStatusList0 = async (req, res) => {
    try {
        const commissionIds = req.body.commissionId;
        const commissionStatuses = req.body.commissionStatus;
        ...
        for (let i = 0; i < commissionIds.length; i++) {
            ...
            try {
                // Try converting the commissionId to ObjectId
                commissionId = new mongoose.Types.ObjectId(commissionIds[i]);
            } catch (error) {
                // Handle the error if the conversion fails
                console.error(`Invalid commissionId: ${commissionIds[i]}`);
                continue; // Skip to the next iteration
            }

            const newCommissionStatus = commissionStatuses[i];

            try {
                // Update the MongoDB record based on the commission ID
                await QuantityBasedCommission.findByIdAndUpdate(commissionId, { commissionStatus: newCommissionStatus });
            } catch (error) {
                console.error(`Error updating commission with ID ${commissionId}:`, error);
                // Handle the error if the update fails
            }
        }

        res.redirect('/admin/quantityBasedCommissionList');
    } catch (error) {
        console.error(error);
        res.status(500).send('Internal Server Error');
    }
