...
const quantityBasedCommissionHistoryListPage = async (req, res) => {
    try {
        ...
        if (searchQuery && searchField) {
            switch (searchField) {
                ...
                case 'commissionAmount':
                    ...
                    if (isNaN(commissionAmount)) {
                        req.flash('warning', 'Invalid payment amount format. Please enter a valid number.');
                        return res.status(200).redirect('/admin/quantityBasedCommissionHistoryList');
                    }
        ...
        const pagination = {
            prev: page > 1 ? `/admin/quantityBasedCommissionHistoryList?page=${page - 1}&search=${searchQuery || ''}&searchField=${searchField || ''}` : null,
            next: page < totalPages ? `/admin/quantityBasedCommissionHistoryList?page=${page + 1}&search=${searchQuery || ''}&searchField=${searchField || ''}` : null,
            current: page,
            totalPages: totalPages,
        };
        ...
        // Render your EJS template with the targetCommission
        res.status(200).render('admin/quantityBasedCommissionHistoryList', {
            targetCommission,
            username,
            totalNotClaimedCommissionQty,
            totalNotClaimedCommissionAmount,
...
const deliveryCommissionListHistoryPage = async (req, res) => {
    try {
        ...
        if (searchQuery && searchField) {
            switch (searchField) {
                ...
                case 'commissionAmount':
                    ...
                    if (isNaN(commissionAmount)) {
                        req.flash('warning', 'Invalid payment amount format. Please enter a valid number.');
                        return res.status(200).redirect('/admin/deliveryCommissionHistoryList');
                    }
        ...
        const pagination = {
            prev: page > 1 ? `/admin/deliveryCommissionHistoryList?page=${page - 1}&search=${searchQuery || ''}&searchField=${searchField || ''}` : null,
            next: page < totalPages ? `/admin/deliveryCommissionHistoryList?page=${page + 1}&search=${searchQuery || ''}&searchField=${searchField || ''}` : null,
            current: page,
            totalPages: totalPages,
        };
        ...
        // Render your EJS template with the targetCommission
        res.status(200).render('admin/deliveryCommissionHistoryList', {
            targetCommission,
            username,
            totalNotClaimedCommissionQty,
            totalNotClaimedCommissionAmount,
