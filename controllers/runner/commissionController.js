...
const deliveryCommissionListHistoryPage = async (req, res) => {
    try {
            ...
            switch (searchField) {
                ...
                case 'commissionAmount':
                    ...
                    if (isNaN(commissionAmount)) {
                        ...
                        return res.status(200).redirect('/runner/deliveryCommissionHistoryList');
                    }
        ...
        const pagination = {
            prev: page > 1 ? `/runner/deliveryCommissionHistoryList?page=${page - 1}&search=${searchQuery || ''}&searchField=${searchField || ''}` : null,
            next: page < totalPages ? `/runner/deliveryCommissionHistoryList?page=${page + 1}&search=${searchQuery || ''}&searchField=${searchField || ''}` : null,
            current: page,
        ...
        res.status(200).render('runner/deliveryCommissionHistoryList', {
            targetCommission,
            username,
